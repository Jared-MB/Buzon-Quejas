...
export function CardSubtitle({ children }: { children: React.ReactNode }) {
	return <h4 className="text-xl font-medium text-zinc-700">{children}</h4>;
}
...
export function CardDescription({
	...
	return (
		<p className={cn("text-zinc-700/60 text-sm text-pretty", className)}>
			{children}
		</p>
	);
}
...
export function CardFooter({
	...
	return (
		<footer
			className={cn(
				"bg-zinc-300/50 rounded-b px-6 py-5 flex flex-row items-center",
				className,
			)}
		>
			{children}
